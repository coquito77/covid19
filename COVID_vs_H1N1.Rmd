...
if(TRUE) {
  ...
  get_breakPoints = function(sel_name) {
  ...
  dt_breakpoints <- lapply(unique(countyTrends$fips), get_breakPoints) %>% 
...
##calculate the regression coefficient r^2
    ...
    dt_breakpoints %>% 
      ...
      left_join(dt_breakpoints %>% arrange(desc(date)) %>%
...
p1
  ...
  dt_breakpoints %>% 
    ...
    geom_vline(data = verticalLines
               ...
               color = "blue", size = .5
               ,alpha = .8) +
    geom_vline(data = dt_breakpoints %>% 
                 select(breakpoints, county, date ) %>% 
                 filter(!is.na( breakpoints))
               ,aes(xintercept = date),
               color = "blue", size = .5
               ,alpha = .5) +
    geom_text(data = dt_breakpoints %>% 
                select(breakpoints, county, date ) %>% 
                filter(!is.na( breakpoints)) %>% 
                left_join( dt_breakpoints %>% 
                             select(county, count) %>% 
                             group_by(county) %>% 
                             arrange(desc(count)) %>% 
  ...
  scale_y_continuous(label = human_numbers
                     ,expand = expansion(mult = c(0.1, .25))) +
    labs(title = "Structural change analysis of daily cases"
...
read_html(hospData_url) %>% 
  ...
  mutate(collection_week = as.Date(collection_week),
         across(contains("and_suspected_covid"), ~ as.numeric(.)),
         across(contains("and_suspected_covid"), ~ na_if(., -999999.0))) %>% 
  group_by(collection_week, NAME) %>% 
  mutate(cln_avg_totl_adlt_ptnt_hspt_cnfr_and_sspc_covd = total_adult_patients_hospitalized_confirmed_and_suspected_covid_7_day_sum /
           total_adult_patients_hospitalized_confirmed_and_suspected_covid_7_day_coverage,
         
         cln_avg_stff_icu_adlt_ptnt_cnfr_and_sspc = staffed_icu_adult_patients_confirmed_and_suspected_covid_7_day_sum /
           staffed_icu_adult_patients_confirmed_and_suspected_covid_7_day_coverage) %>% 
  select(collection_week:NAME, NAME, starts_with("cln_")) %>% 
  summarize(cln_avg_totl_adlt_ptnt_hspt_cnfr_and_sspc_covd = sum(cln_avg_totl_adlt_ptnt_hspt_cnfr_and_sspc_covd
                                                                 , na.rm = TRUE),
            cln_avg_stff_icu_adlt_ptnt_cnfr_and_sspc = sum( cln_avg_stff_icu_adlt_ptnt_cnfr_and_sspc
                                                            , na.rm = TRUE)) %>% 
  ungroup %>% 
