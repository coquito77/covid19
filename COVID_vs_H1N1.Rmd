...
if(FALSE) system(paste0('open "', "covid19DataNYTimes.R", '"'))

source("helperFunctions.r")

cols <- c("Cases" = "#d8b365", "Deaths" = "#998ec3", "Hospitalizations" = "#5ab4ac")

sum_US_CoronaVirus <- US_StCoronaVirus %>% 
  group_by(date) %>% 
...
if( file.exists( paste0("2019H1N1_vs_COVID19_"
                        ,data_Date %>% gsub("-","",.),".pdf"))  )   unlink( paste0("2019H1N1_vs_COVID19_"
                                                                                   ,data_Date %>% gsub("-","",.),".pdf")  )
...
sum_US_CoronaVirus %>% 
  ggplot() +
    geom_text(data = data.frame(x = 10, y = 2) 
            , aes(x = x,
                  y = y,
                  label = "Note: Chart is in logaritmic scale. Vertical scale is compress to show changes in percentage terms.
                  A rise from 100 to 110 takes the same space as a rise from 100,000 to 110,000 since both are 10% changes."
            )
            ,hjust = 0, nudge_x = 1
            ,colour = "black",
            size = 3 ) +
  ...
  geom_line(aes(x = dayCnt
                ,y = totalDailyCases),
            colour = "red") +
  geom_text(data = H1N1_Case_Max # H1N1_Case_currentDay 
            , aes(x = dayCnt,
                  y = imp_MidLevelRange,
            ...
            , hjust = 0, nudge_x = 1
            ,colour = "blue") +
  geom_point(data =  H1N1_Case_Max # H1N1_Case_currentDay 
             , aes(x = dayCnt,
                   y = imp_MidLevelRange)
             ,colour = "blue") +
  ...
  theme_plot +
  coord_cartesian(xlim = c(1, Cov_currentDay$dayCnt+80)
...
sum_US_CoronaVirus %>% 
  ...
  geom_point(data = Cov_currentDay
             ...
             ,colour = "red") +
  theme_plot +
  coord_cartesian(xlim = c(1, max(Cov_currentDay$dayCnt)+80)
                  , ylim = c(1, H1N1_Case_Max$imp_highEst)) +
...
sum_US_CoronaVirus %>% 
  ...
  scale_fill_manual(name = "Type",
                    values = cols) +
  geom_text(data = H1N1_Death_Max# H1N1_Death_currentDay 
            , aes(x = dayCnt,
                  y = imp_MidLevelRange,
            ...
            , hjust = 0, nudge_x = 1
            ,colour = "blue") +
  geom_point(data =  H1N1_Death_Max# H1N1_Death_currentDay 
             , aes(x = dayCnt,
                   y = imp_MidLevelRange)
             ,colour = "blue") +
  geom_text(data = Cov_currentDay 
...
coronaChange <- coronaChange %>% 
  full_join( expand.grid(dayCnt = seq(min(coronaChange$dayCnt)
                                      ,max(coronaChange$dayCnt), by = 1),
                         type = c("cases", "deaths") ) %>% 
               group_by(type) %>% 
               mutate( date = seq(max(coronaChange$date)+1, by = "1 day"
                                  ,length.out = max(coronaChange$dayCnt)-max(coronaChange$dayCnt)+1
               )) %>% ungroup  
  ) %>% 
...
coronaChange %>% 
  bind_rows(H1N1Change) %>% 
  filter(type == "cases") %>% 
  mutate(dataSet = case_when(is.na(dataSet) ~ "COVID-19",
                             TRUE ~ dataSet)) %>% 
  ggplot() +
  ...
  labs(title = "2009 H1N1 new cases by day vs Covid-19 cases"
       ,subtitle = "Vertical blue lines are for reference and placed every two months
...
countyTrends <- US_CoronaVirus %>% 
  ...
  mutate(newCases = lag(cases),
         ...
         newDeaths = lag( deaths),
         newDeaths = deaths - newDeaths) %>% 
  ungroup() %>% 
  as.data.table() %>% 
...
# https://sites.uci.edu/energyobserver/2020/04/11/doubling-days-for-a-given-daily-percent-increase/
# http://rstudio-pubs-static.s3.amazonaws.com/10369_9e0e4a116538489baa18908c1690293c.html
# https://data.princeton.edu/eco572/grdt
# https://rpubs.com/anthonybmasters/doubling-times


countyTrends %>% 
  filter(variable == "rollingMean07Day") %>% 
  mutate(count = replace_na(count, 0)) %>% 
  group_by(fips) %>% 
  mutate(cases_doub_days = 1*log(2)/log(count/lag(count, 1)),
         cases_doub_days = case_when(cases_doub_days < 0 ~ NA_real_,
                                     cases_doub_days > 35 ~ 35, 
                                     TRUE ~ cases_doub_days)) %>% 
  ungroup %>% 
  ggplot() + 
  geom_col(aes(x = date, y = cases_doub_days)) +
...
ggrepel::geom_text_repel(data = countyTrends %>% 
                         ...
                         ,aes(x =  date #+ 20
                              , y = cases_doub_days
                              # color = variable,
                              ,label = paste0(scales::comma(cases_doub_days, accuracy = 0.1), " days")
                         )
                         ,show.legend = FALSE,
                         size = 3,
                         force             = 0.5,
  ...
  labs(title = "Days to double the number of cases"
       ,subtitle = "based on seven day rolling average of new cases,
       for counts increase only, if the counts double in more
       than 35 days they show as 35 days."
       ,y = "Cases"
       ,x = ""
       ,caption = paste("Sources: The New York Times, https://github.com/nytimes/covid-19-data accessed"
                        ,data_Date
...
if(TRUE) {
  ...
  get_cases_breakPoints = function(sel_name) {
    
    # sel_name <- 6037
    
    dum <- countyTrends %>% 
      filter(variable == "newCases", fips == sel_name) %>% 
      right_join(data.frame(date = seq( min(.$date), max(.$date), by ="day" ))) %>% 
      mutate(count = replace_na(count, 0))
    
    tsDaily <- ts(dum$count, frequency = 365
                  , start = c(year(dum$date)[1], as.numeric(format(dum$date, "%j")[1])))
    
    # complete structural change analysis on this variable
    
    breakTrendData <- tsDaily
    
    # plot(breakTrendData)
    bpData <- breakpoints(breakTrendData ~ 1)
    summary(bpData)
    
    breakPoints <- confint(bpData) %>% 
      .$confint %>% 
      as.data.frame() %>% 
      left_join(dum %>% mutate(breakpoints = format(dum$date, "%j") %>% as.numeric))
    
    return(breakPoints)
  }
  
  get_deaths_breakPoints = function(sel_name) {
    
    # sel_name <- 6037
    
    dum <- countyTrends %>% 
      filter(variable == "newDeaths", fips == sel_name) %>% 
      right_join(data.frame(date = seq( min(.$date), max(.$date), by ="day" ))) %>% 
      mutate(count = replace_na(count, 0))
  ...
  dt_cases_breakpoints <- lapply(unique(countyTrends$fips), get_cases_breakPoints) %>% 
    data.table::rbindlist(., fill = TRUE, idcol= "iteration") %>% 
    right_join(countyTrends %>% 
                 filter(variable == "newCases")) %>% # dt_breakpoints %>% 
    left_join( countyTrends %>% 
                 group_by(fips) %>% 
                 arrange(date) %>% 
                 distinct(fips,.keep_all = TRUE ) %>% 
                 select(date, fips) %>% 
                 mutate(breaks = 1) %>% ungroup ) %>%
    arrange(fips, date) %>% 
    group_by(fips) %>% 
    mutate(breaks = case_when(!is.na(breakpoints) ~ 1, 
                              TRUE~ breaks),
           breaks = replace_na(breaks, 0),
           breaks = cumsum(breaks),
           breaks = as.character(breaks)) %>% 
    ungroup
  
  dt_deaths_breakpoints <- lapply(unique(countyTrends$fips), get_deaths_breakPoints) %>% 
    data.table::rbindlist(., fill = TRUE, idcol= "iteration") %>% 
    right_join(countyTrends %>% 
                 filter(variable == "newDeaths")) %>% # dt_breakpoints %>% 
    left_join( countyTrends %>% 
                 group_by(fips) %>% 
                 arrange(date) %>% 
                 distinct(fips,.keep_all = TRUE ) %>% 
...
##calculate the regression coefficient r^2
    dt_cases_breakpoints %>% 
      as.data.table() %>% 
      .[, summary(lm(count~date))$r.squared, keyby = .(breaks, fips, county)]
    
    dt_cases_breakpoints %>% 
      as.data.table() %>% 
      .[,list(coef_Intercept = coef(summary(lm(count~date)))[1],
              coef_x = coef(summary(lm(count~date)))[2],
              cof_Estimate = summary(lm(count~date))$coefficients[1],
              cof_Std_err = summary(lm(count~date))$coefficients[2],
              cof_t_val = summary(lm(count~date))$coefficients[3],
              cof_t_stat = summary(lm(count~date))$coefficients[4],
              r2 = summary(lm(count~date))$r.squared ,
              f = summary(lm(count~date))$fstatistic[1] )
        ,keyby = .(breaks, fips, county)] %>% 
      .[order(-county, -breaks, decreasing = TRUE),] %>% 
      left_join(dt_cases_breakpoints %>% arrange(desc(date)) %>%
                  distinct(county, breaks, .keep_all = TRUE) %>% 
                  select(date, county, breaks)) %>% 
      mutate(estimate = coef_Intercept + (coef_x * as.numeric(date)))
...
p1
  ...
  plot_Strc_Chng_cases <- dt_cases_breakpoints %>% 
    ggplot() +
    aes(x = date, y = count) +
    geom_line(stat = "identity", size = .5, color = "#636363") + 
    ...
    geom_smooth(se = FALSE, method = lm, linetype = 'twodash',
                aes(group = breaks, show.legend = FALSE)
    )  +
    guides(colour = FALSE) +
    geom_vline(data = verticalLines
               ,aes(xintercept = vline),
               color = "blue", size = .5
               ,alpha = .8) +
    geom_vline(data = dt_cases_breakpoints %>% 
                 select(breakpoints, county, date ) %>% 
                 filter(!is.na( breakpoints))
               ,aes(xintercept = date),
               color = "blue", size = .5
               ,alpha = .5) +
    geom_text(data = dt_cases_breakpoints %>% 
                select(breakpoints, county, date ) %>% 
                filter(!is.na( breakpoints)) %>% 
                left_join( dt_cases_breakpoints %>% 
                             select(county, count) %>% 
                             group_by(county) %>% 
                             arrange(desc(count)) %>% 
    ...
    scale_y_continuous(label = human_numbers
                       ,expand = expansion(mult = c(0.1, .25))) +
    labs(title = "Structural change analysis of daily new cases"
         ,subtitle = "blue lines with dates reflect when there was a break in the trend for each county"
         ,y = "Cases"
         ,x = ""
         ,caption = paste("Note: new break trends can developed as new daily counts are included.
                          Sources: The New York Times, https://github.com/nytimes/covid-19-data accessed"
                          ,data_Date
                          ,"(Covid19)")
    )
  
  print(plot_Strc_Chng_cases)
  
 plot_Strc_Chng_deaths <- dt_deaths_breakpoints %>% 
    ggplot() +
    aes(x = date, y = count) +
    geom_line(stat = "identity", size = .5, color = "#636363") + 
    geom_point(alpha = 1/2, size = .5) +
    ...
    geom_vline(data = verticalLines
               ...
               color = "blue", size = .5
               ,alpha = .8) +
    geom_vline(data = dt_deaths_breakpoints %>% 
                 select(breakpoints, county, date ) %>% 
                 filter(!is.na( breakpoints))
               ,aes(xintercept = date),
               color = "blue", size = .5
               ,alpha = .5) +
    geom_text(data = dt_deaths_breakpoints %>% 
                select(breakpoints, county, date ) %>% 
                filter(!is.na( breakpoints)) %>% 
                left_join( dt_deaths_breakpoints %>% 
                             select(county, count) %>% 
                             group_by(county) %>% 
                             arrange(desc(count)) %>% 
                             distinct(county, .keep_all = TRUE ) )
    ...
    scale_y_continuous(label = human_numbers
                       ,expand = expansion(mult = c(0.1, .25))) +
    labs(title = "Structural change analysis of daily new deaths"
         ,subtitle = "blue lines with dates reflect when there was a break in the trend for each county"
         ,y = "Cases"
         ,x = ""
         ,caption = paste("Note: new break trends can developed as new daily counts are included.
...
if( !file.exists("cb_2018_us_county_500k.Rdata")) {
  ...
  cb_2018_us_zcta510_500k <- list.files(recursive = TRUE
                                        ,pattern = "shp$") %>% 
    .[grepl(fname,.)] %>% 
    sf::read_sf(quiet = TRUE, stringsAsFactors = FALSE) %>% 
    sf::st_transform(., 4269)
  
  sf::st_crs(cb_2018_us_zcta510_500k)
  
  save(cb_2018_us_zcta510_500k, file = "cb_2018_us_zcta510_500k.Rdata")
  
}

  # add hospital beds ICU and covid
  #  https://healthdata.gov/dataset/covid-19-reported-patient-impact-and-hospital-capacity-facility

load("cb_2018_us_zcta510_500k.Rdata")
load("cb_2018_us_county_500k.Rdata")

zipcodes <- st_intersection(  cb_2018_us_zcta510_500k %>% 
                                mutate(geo_all_SqrMile = st_area(geometry),
                                       geo_all_SqrMile = as.numeric(geo_all_SqrMile),
                                       geo_all_SqrMile = geo_all_SqrMile / 2.59e+6) %>% 
                                # rmapshaper::ms_simplify() %>% 
                                select(zip = ZCTA5CE10 , geo_all_SqrMile)
                              ,
                              cb_2018_us_county_500k %>%
                                filter(STATEFP == "06") %>%
                                filter(grepl("Los Angeles|Riverside|Orange|San Bernardino", NAME)) %>% 
                                select( STATEFP, COUNTYFP, NAME)
) %>% 
  st_sf %>% 
  mutate(intersectLACo = 1,
         geo_intersect_SqrMile = st_area(geometry),
         geo_intersect_SqrMile = as.numeric(geo_intersect_SqrMile),
         geo_intersect_SqrMile = geo_intersect_SqrMile / 2.59e+6,
         pcnt_Intr_ovr_All = geo_intersect_SqrMile/geo_all_SqrMile,
         pcnt_Intr_ovr_All = round(pcnt_Intr_ovr_All, 2),
         geo_intersect_SqrMile = round(geo_intersect_SqrMile, 2),
         #geo_intersect_SqrMile = round(geo_intersect_SqrMile, 2),
         geo_all_SqrMile = round(geo_all_SqrMile, 2)
  ) %>% 
  group_by(zip) %>% 
  mutate(cnt_splits = n()) %>% 
  ungroup

rm(list = c("cb_2018_us_zcta510_500k", "cb_2018_us_county_500k"))

hospData_url <- "https://healthdata.gov/dataset/covid-19-reported-patient-impact-and-hospital-capacity-facility"

read_html(hospData_url) %>% 
  html_nodes(., "a") %>% 
  html_attr(., "href") %>% 
  .[grepl(".csv$", .)] %>% 
  fread() %>% 
  # filter(grepl("CA", state, ignore.case = TRUE)) %>% 
  # filter(grepl("Riverside", city, ignore.case = TRUE)) %>% 
  # select(state, city, hospital_name, zip) %>% 
  mutate(zip = as.character(zip)) %>% 
  inner_join(  zipcodes %>% st_drop_geometry() %>%
                 arrange(desc(pcnt_Intr_ovr_All)) %>% distinct(zip, .keep_all = TRUE) %>% #View("newZips") 
                 select(zip, NAME)
  ) %>% # tmp %>% 
  select(collection_week, city, hospital_name, NAME, total_icu_beds_7_day_sum, icu_beds_used_7_day_sum ) %>% 
  #   select(collection_week, city, hospital_name, NAME, contains("staffed_icu_adult_patients_confirmed_and_suspected")
  #          ,contains("total_adult_patients_hospitalized_confirmed_and_suspected_c")
  mutate(collection_week = as.Date(collection_week),
         across(contains("_7_day_sum"), ~ na_if(., "-999999")),
         across(contains("_7_day_sum"), ~ as.numeric(.)) 
  ) %>% 
  group_by(collection_week, NAME) %>% 
  mutate(cln_icu_beds_ovr_total = icu_beds_used_7_day_sum /
           total_icu_beds_7_day_sum ) %>% 
  select(collection_week:NAME, NAME, starts_with("cln_")) %>% 
  summarize(cln_icu_beds_ovr_total = sum(cln_icu_beds_ovr_total
                                                                 , na.rm = TRUE)) %>% 
  ungroup %>% 
  pivot_longer(-c(collection_week,NAME) ) %>% 
  ggplot() +
  geom_line(aes(x =  collection_week, y = value, color = name, group = name)
            , show.legend = FALSE) +
  geom_point(aes(x =  collection_week, y = value, color = name)
             , show.legend = FALSE) +
  scale_y_continuous(label = human_numbers
                     ,expand = expansion(mult = c(0.1, .25))) +
  facet_wrap(vars(NAME, name), labeller = "label_both", scales = "free_y", ncol = 2) +
  theme_plot +
  labs(title = "COVID-19 Reported Patient Impact and Hospital Capacity by Facility"
       ,subtitle = "For Los Angeles, Orange, Riverside and San Bernardino Counties"
       ,y = "Cases"
       ,x = ""
       ,caption = paste("Sources: U.S. Department of Health and Human Services Office\n",hospData_url, "accessed"
                        ,data_Date)
  )
...
if(FALSE) {
  ...
  wklyCnts_t  %>% # some pcnts are dif eg week 2017-48 has more than one value reported
    arrange(desc(iteration)) %>%
    distinct(Finalstart, final_MMWRWeek, .keep_all = TRUE) %>%
    ggplot() +
    geom_line( aes(x = final_MMWRWeek, y = PcntNum
                   ,color = FinalstartWeekYear
                   ,group = FinalstartWeekYear
    ),
    alpha = .5) +
