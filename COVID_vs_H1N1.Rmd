...
cntyList <- data.frame(county = c(6071,6059,6065, 6037),
                       cntyLabel= c("San Bernardino",
                                    "Orange","Riverside",
                                    "Los Angeles")
)

for(i in seq_along(cntyList$county) ) {
  
  dat <- US_CoronaVirus %>% 
    filter(fips == cntyList$county[i]) %>% 
    ...
    ungroup() %>% 
    as.data.table() %>% 
    .[, rollingMean07Day := frollmean(.[, newCases], 7) ] %>%
    .[, rollingMean14Day := frollmean(.[, newCases], 14) ] %>%
    select(date, county, newCases, contains("rolling")) %>%
    pivot_longer(-c(date, county), names_to = "cat", values_to = "count") %>% 
    mutate(count = round(count, 0))
  
  maxRollingValues <- dat %>% 
    filter(cat != "newCases") %>% 
    group_by(cat) %>% 
    filter(date == max(date)) %>% 
    ungroup
  ...
  gPlot <- ggplot() +
    ...
    geom_text(data = maxRollingValues 
              ,aes(x = date, y = count, color = cat
                   ,label = scales::comma(count, accuracy = 1)),
              size = 3, show.legend = FALSE
              ,alpha = .8) +
    labs(title = maxRollingValues[1,2]
         ,subtitle = "New cases, rolling average 7 and 14 days"
         ,y = "Cases"
         ,x = ""
         ,caption = paste("Sources: The New York Times, https://github.com/nytimes/covid-19-data accessed"
                          ,data_Date
...
p +
  ggrepel::geom_text_repel(data = countyPop %>% 
                           ...
                           ,aes(x =  date #+ 20
                                , y = casesPer1K,
                                color = state1,
                                label = paste(state_abb,scales::comma(casesPer1K)
                                )
                           )
                           ,show.legend = FALSE,
                           size = 3,
...
# now by county near Los Angeles
...
selectedCounties <- "Ventura|Santa Barbara|San Francisco|San Bernardino|Riverside|Kern|Imperial|San Diego|Orange|Los Angeles|Kings|Tulare|Marin"
