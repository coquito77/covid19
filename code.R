...
if(file.exists("tigerroads.Rdata")) {
  ...
  tigerroads = sf::st_read("./shapefiles/tigerroads.shp") %>% 
    ...
    st_transform(., 2229) %>% # t2000 <- tigerroads %>% # change coordinte system to LA County official system
...
Cairo::CairoPDF(file = "LACountyCOVID19_20200328PM.pdf",
                #units = "in", dpi = 150,
                width = 8, 
                height = 11, 
                pointsize = 10)
...
system(paste0('open "', 'LACountyCOVID19_20200328PM.pdf', '"'))
